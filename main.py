...
if __name__ == "__main__":
    """
    Name of the variables the API must return: 
        - shoot_coordinate 
        - head_centroid
        - pick_up_coordinate
    """
    ...
    # image Reading stuff
    image = os.path.join(project_dir, "Data", "sperm_cut.jpg")
    ...
    # LOAD THE SEGMENTATION WEIGHTS  
    # Step 1
    seg_model = load_seg_model(seg_weights_path)
    # ADD THE PADDING NECESSARY TO STANDARIZE THE IMAGE DIMENSIONS TO THE MODEL CAN SEGMENT PROPERLY 
    # Step 2
    padded_image = pad_image(gs_img, cut_size)
    # SEND THE PADDED IMAGE TO SEGMENT 
    # Step 3
    segmented_sperm = seg_sperm(padded_image, seg_model)
    # REMOVE THE PADDING ADDED IN STEP 2 TO THE ORIGINAL CUT SENT AND ITS MASK
    # Step 4
    resized_image, resized_mask = crop_image(padded_image, segmented_sperm, gs_img)
    # GET THE SHOOT COORDINATE
    #Step 5
    bbx_centroid, shoot_coordinate, tail_coordinates = get_shoot_coords(resized_mask)
    # GET THE HEAD CENTROID COORDINATES AND THE PICK-UP COORDINATES 
    # Step 6
    pick_up_coordinate, head_centroid = get_pickup_coordinate(resized_mask, tail_coordinates)
